...
from RecommendSys import PushFood, PushFood2, LunchPush
...
def LunchPush(event):
    ...
    # 計算一餐的量
    # 午餐
    Kcal_lunch = FTDEE * 0.4
    CHO_lunch = round(CHO_day * 0.4)
    protein_lunch = round(protein_day * 0.4)
    Fat_lunch = round(Fat_day * 0.4)
    # print("Lunch:", "Kcal=", Kcal_lunch, "CHO_g=", CHO_lunch, "g,", "Pro_g=", protein_lunch, "g,", "Fat_g=", Fat_lunch, "g")
    ...
    condition2 = df['Lunch'].astype(float) == 1
    condition3 = df['FoodLabel'].astype(float) == 1
    condition4 = df['Cal_kcal'].astype(float) <= Kcal_lunch
    condition5 = (df['CHO_g'].astype(float) > CHO_lunch * 0.5) & (df['CHO_g'].astype(float) <= CHO_lunch)
    condition6 = (df['Fat_g'].astype(float) > Fat_lunch * 0.5) & (df['Fat_g'].astype(float) <= Fat_lunch)
    condition7 = df['Protein_g'].astype(float) > protein_lunch * 0.5
    ...
    show1 = df2.sample(n=3, replace=False, random_state=None, axis=None)
    print(show1[['Braname']])
    print("-----------------------------")

    details = show1.loc[:, ['Braname', 'Intake_g', 'Cal_kcal', 'CHO_g', 'Fat_g', 'Protein_g']]
